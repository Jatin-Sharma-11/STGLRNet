...
class GC2L(nn.Module):
    def __init__(self, in_features, hidden_features, out_features, num_layers=3, dropout=0.3):
        ...
        # First layer
        self.layers.append(GC2(in_features, hidden_features))
        self.norms.append(nn.LayerNorm(hidden_features))

        # Intermediate layers
        for _ in range(num_layers - 2):
            self.layers.append(GC2(hidden_features, hidden_features))
            self.norms.append(nn.LayerNorm(hidden_features))

        # Last layer (no norm needed if not followed by activation)
        self.layers.append(GC2(hidden_features, out_features))
